...
Core logic for Chim-eTunes Music Bot (Desktop Version).
Contains:
...
- "intent": one of ["create_playlist", "show_playlists", "analyze_playlist", "add_to_playlist", "remove_from_playlist", "get_recommendations", "read_playlist", "get_artist_details", "get_track_details", "casual_chat"]
  * "show_playlists": when user wants to see their own playlists (e.g., "show my playlists", "list my playlists", "what playlists do I have")
  * "read_playlist": when user provides a Spotify playlist URL/link to read
  ...
  * "get_recommendations": when user wants recommendations based on a specific genre or mood (e.g., "recommend 5 pop songs", "give me 10 k-pop songs")
...
- "target_genre": (string) for genre names. Do NOT include region prefixes here, EXCEPT for these specific known genres:
  * "Korean pop" or "kpop" or "k-pop" -> "k-pop"
  * "Filipino pop" or "opm" -> "opm"
  * "Japanese pop" or "jpop" or "j-pop" -> "j-pop"
  * "Japanese rock" or "j-rock" -> "j-rock"
  * For other genres, use the base genre (e.g. "japanese jazz" -> "jazz")
- "target_playlist": (string) name or ID of playlist to show or analyze. Examples:
  * "show my playlists" -> ""
  * "analyze my chill vibes playlist" -> "chill vibes"
  * "recommend songs like my workout playlist" -> "workout"

Only return valid JSON. Example:
{{"intent":"get_recommendations","playlist_url":"","artist":"","limit":10,"playlist_name":"","seed_artist":"Joji","seed_track":"","add_mode":"","search_query":"","target_genre":""}}
"""
    ...
    def generate_response(self, history_text, extra_instructions=""):
...
class ChatBot:
    ...
    def handle_message(self, user_text, history, session_data):
        ...
        try:
            if intent == "create_playlist":
                ...
                reply_text = self.parser.generate_response(history_text)
    ...
    def _handle_add_to_playlist(self, parsed, session_data):
        ...
        if not pid: return "Please provide a valid playlist link.", []
    ...
    def _handle_remove_from_playlist(self, parsed):
        ...
        if artist:
            tracks = self.sp.get_playlist_tracks(pid)
            uris_to_remove = [t['uri'] for t in tracks if artist.lower() in [art['name'].lower() for art in t['artists']]]
            if uris_to_remove:
                self.sp.remove_tracks(pid, uris_to_remove)
                return f"Removed {len(uris_to_remove)} tracks by {artist}.", []
            return f"No tracks by {artist} found.", []
        return "Please specify artist(s) to remove.", []

    def _handle_read_playlist(self, parsed):
        if not self.sp: return "You need to log in with Spotify to read playlists.", []
        
        pid = parse_playlist_id(parsed.get("playlist_url"))
        if not pid: return "Invalid playlist link.", []
        
        tracks = self.sp.get_playlist_tracks(pid)
        items = [f"{t['name']} by {t['artists'][0]['name']}" for t in tracks]
        return f"Found {len(items)} tracks.", items
    ...
    def _handle_recommendations(self, parsed, session_data):
        ...
        # --- Local Mode Logic ---
        if search_query:
            res = self.local.search(search_query, limit=limit)
                ...
                for mood, features in mood_keywords.items():
                    if mood in query_lower:
                        ...
                        if not filtered_df.empty:
                            sample = filtered_df.sample(n=min(limit * 2, len(filtered_df))) # Get more for filtering
                    ...
                    if artist_tracks:
                        ...
                        if matched_features:
                            ...
                            if not artist_df.empty:
                                ...
                                sample = artist_df.sample(n=min(limit, len(artist_df)))
                                recs = self.local._to_spotify_format(sample)
                                msg = f"🎵 Here are some {search_query} by {seed_artist}:"
                            else:
                                # No tracks match the mood criteria, return top tracks
                                recs = artist_tracks[:limit]
            ...
            res = self.local.get_recommendations(seed_genres=[target_genre], limit=limit)
            recs = res['tracks']
            
            # If no results and it's a culture-specific genre, try searching by known artists
            if not recs and target_genre in ['opm', 'k-pop', 'j-pop']:
                # Map genre to known artists
                ...
                nationality = genre_map.get(target_genre)
                if nationality and nationality in KNOWN_ARTISTS:
                    # Search for tracks by any of the known artists
                    all_tracks = []
                    for artist in KNOWN_ARTISTS[nationality][:10]:  # Limit to first 10 artists
                        artist_res = self.local.search(artist, type='track', limit=2)
                        if artist_res['tracks']['items']:
                            # Filter to only exact artist matches (case-insensitive)
                            for track in artist_res['tracks']['items']:
                                track_artist = track['artists'][0]['name'].lower()
                                # Check if the known artist name is in the track artist
                                # But also check it's not a partial match (e.g., "bamboo" vs "the bamboos")
                                if artist.lower() in track_artist:
                                    # Additional check: ensure it's the main artist, not just a substring
                                    # Split by common separators and check if artist matches any part
            ...
            if target_track:
                ...
                r = self.local.get_recommendations(seed_tracks=[tid], limit=limit)
                recs = r['tracks']
            ...
            if res['artists']['items']:
                ...
                res = self.local.search(seed_artist, type='track', limit=limit)
                recs = res['tracks']['items']
            ...
            if available_genres:
                ...
                tracks_per_genre = max(2, limit // len(available_genres))
                ...
                sample = self.local.df.sample(n=min(limit, len(self.local.df)))
                recs = self.local._to_spotify_format(sample)
        ...
        if recs:
            ...
            for t in recs:
                ...
                if 'similarity_score' in t:
                    ...
                    similarity_pct = int(t['similarity_score'] * 100)
